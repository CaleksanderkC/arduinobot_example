...
class SimpleParameter(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('simple_string_param', "Arduio")
    ...
    def paramChangeCallback(self, params):
        ...
        for param in params:
            if param.name == "simple_int_param" and param.type_ == Parameter.Type.INTEGER:
                self.get_logger().info(f"Param simple_int_param_change! New value: {param.value}")
                result.successful = True
            if param.name == "simple_string_param" and param.type_ == Parameter.Type.STRING:
                self.get_logger().info(f"Param simple_string_param! New value: {param.value}")
                result.successful = True
