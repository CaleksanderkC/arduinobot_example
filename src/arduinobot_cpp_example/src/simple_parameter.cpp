...
class SimpleParameter : public rclcpp::Node
{
    public:
        SimpleParameter() : Node("simple_parameter")
        {
            ...
            declare_parameter<std::string>("simple_string_param", "Arduino");

            param_callback_handler = add_on_set_parameters_callback(std::bind(&SimpleParameter::paramChangeCallback, this, _1));
        }

    private:
        OnSetParametersCallbackHandle::SharedPtr param_callback_handler;
        ...
        rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> parameters)
        {
            ...
            for (const auto& param : parameters)
            {
                ...
                {
                    RCLCPP_INFO_STREAM(get_logger(), "Param simple_int_param change! New Value is: " << param.as_int() );
                    result.successful = true;
                }

                if(param.get_name() == "simple_string_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
                {
                    RCLCPP_INFO_STREAM(get_logger(), "Param simple_string_param change! New Value is: " << param.as_string());
                    result.successful = true;
                }
            }
